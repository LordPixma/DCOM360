...
// Base API endpoint (fallback to production worker URL if not supplied)
const WORKER_BASE = (globalThis as any).VITE_API_BASE || 'https://flare360-worker.samuel-1e5.workers.dev'
...
test('map renders container, marker optional', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByText(/Global Map|Interactive Map/i)).toBeVisible()
  const hasToken = Boolean((globalThis as any).VITE_MAPBOX_ACCESS_TOKEN)
  if (hasToken) {
