...
${body}`;
  ...
  const ts = get(/Date:\s*([^\n]+)/i) || (/* @__PURE__ */ new Date()).toISOString();
  ...
  return {
    ...
    coordinates_lat: isFinite(lat) ? lat : void 0,
    coordinates_lng: isFinite(lng) ? lng : void 0,
    event_timestamp: new Date(ts).toISOString(),
    description: description?.slice(0, 2e3)
  };
}
__name(parseEmail, "parseEmail");
function parseEmailMulti(subject, body) {
  const text = `${subject}
${body}`;
  const results = [];
  ...
  const sevMap = /* @__PURE__ */ __name((s) => s.toUpperCase().startsWith("RED") ? "RED" : s.toUpperCase().startsWith("ORANGE") ? "ORANGE" : "GREEN", "sevMap");
  ...
  while (m = eqRe.exec(norm)) {
    ...
    results.push({
      ...
      metadata: { magnitude: parseFloat(mag), depth_km: parseFloat(depthKm), original_country_name: countryName || void 0 }
    });
  }
  ...
  while (m = tcRe.exec(norm)) {
    ...
    const endIso = range ? toIsoFromEuropean(range[2], "00:00") : (/* @__PURE__ */ new Date()).toISOString();
    const countriesMatch = /The cyclone affects these countries:\s*([^\.\n]+)/i.exec(tail);
    const firstCountry = countriesMatch ? countriesMatch[1].split(",")[0].trim() : void 0;
    const countryName = firstCountry ? sanitizeCountry(firstCountry.replace(/\(.*?\)/, "").trim()) : void 0;
    const countryIso = countryName ? resolveCountryIso2(countryName) : void 0;
    const catMatch = /Category\s*([1-5])/i.exec(tail);
    const windMatches = Array.from(tail.matchAll(/(\d{2,3})\s*(?:km\/?h|kph)/gi));
    const maxWind = windMatches.length ? Math.max(...windMatches.map((w) => parseInt(w[1], 10))) : void 0;
    ...
    results.push({
      ...
      metadata: { name, category: catMatch ? parseInt(catMatch[1], 10) : void 0, max_wind_kmh: maxWind, original_country_name: countryName }
    });
  }
  return results.length ? results : [parseEmail(subject, body)];
}
__name(parseEmailMulti, "parseEmailMulti");
function toIsoFromEuropean(dateDDMMYYYY, timeHHMM) {
...
__name(resolveCountryIso2, "resolveCountryIso2");
...
__name(mapAlertLevel, "mapAlertLevel");
...
__name(extractAffectedPopulation, "extractAffectedPopulation");
function parseGdacsFeed(xml) {
  ...
  for (const it of list) {
    ...
    const pub = it.pubDate ? new Date(it.pubDate) : /* @__PURE__ */ new Date();
    const when = isNaN(pub.getTime()) ? /* @__PURE__ */ new Date() : pub;
    const country = it["gdacs:country"] ? String(it["gdacs:country"]) : void 0;
    ...
    if (geopt) {
      ...
      lat = Number(latStr);
      lng = Number(lngStr);
      if (!isFinite(lat)) lat = void 0;
      if (!isFinite(lng)) lng = void 0;
    }
    const description = it.description ? String(it.description) : void 0;
...
__name(parseGdacsFeed, "parseGdacsFeed");
...
__name(inferTypeFromText, "inferTypeFromText");
function extractCategories(it) {
  ...
  if (cats && typeof cats === "object") return [cats["#text"] || cats.text || ""].filter(Boolean);
  return [];
}
...
__name(extractCountryFromDescription, "extractCountryFromDescription");
function extractGlideNumber(description, categories) {
...
__name(extractGlideNumber, "extractGlideNumber");
function extractCountryFromCategories(categories) {
...
__name(extractCountryFromCategories, "extractCountryFromCategories");
...
__name(parseMagnitude, "parseMagnitude");
function parseCycloneCategory(text) {
  const m = /\bcat(?:egory)?\s*([1-5])\b/i.exec(text);
  return m ? parseInt(m[1], 10) : void 0;
}
__name(parseCycloneCategory, "parseCycloneCategory");
function parseNumberNearKeywords(text, keywords) {
  const window2 = 120;
  const matches = [];
  for (const kw of text.matchAll(keywords)) {
    const idx = kw.index ?? 0;
    const slice = text.slice(Math.max(0, idx - window2), Math.min(text.length, idx + window2));
    ...
    if (nums) {
      for (const n of nums) {
        ...
        if (millionWord) {
          ...
          if (isFinite(num)) matches.push(num * 1e6);
          continue;
        }
        ...
        else if (suffix.toLowerCase() === "k") matches.push(val * 1e3);
        else if (suffix.toLowerCase() === "m") matches.push(val * 1e6);
      }
    }
  }
...
__name(parseNumberNearKeywords, "parseNumberNearKeywords");
function extractAffectedPopulation2(text) {
...
__name(extractAffectedPopulation2, "extractAffectedPopulation");
function inferSeverityFromText(text, disasterType) {
  const t = text.toLowerCase();
  ...
  if (disasterType === "flood") {
    if (deaths !== void 0 && deaths >= 50) return "RED";
    if (affected !== void 0 && affected >= 1e5) return "RED";
    if (deaths !== void 0 && deaths >= 10) return "ORANGE";
    if (affected !== void 0 && affected >= 1e4) return "ORANGE";
  }
  if (disasterType === "wildfire") {
    if (/out\s+of\s+control|uncontrolled|evacuations|homes?\s+destroyed/i.test(text)) return "ORANGE";
    if (deaths !== void 0 && deaths >= 5) return "RED";
  }
  if (disasterType === "epidemic") {
    if (deaths !== void 0 && deaths >= 20) return "RED";
    if (affected !== void 0 && affected >= 1e3) return "ORANGE";
    if (/outbreak\s+declared|epidemic|pandemic/i.test(text)) return "ORANGE";
  }
  if (disasterType === "drought") {
    if (affected !== void 0 && affected >= 1e6) return "RED";
    if (affected !== void 0 && affected >= 1e5) return "ORANGE";
  ...
  if (disasterType === "heatwave") {
    if (deaths !== void 0 && deaths >= 10) return "RED";
  ...
  if (injured !== void 0 && injured >= 100) return "ORANGE";
  if (affected !== void 0) {
    if (affected >= 5e5) return "RED";
    if (affected >= 5e4) return "ORANGE";
  }
  if (redWords.test(text)) return "RED";
  if (orangeWords.test(text)) return "ORANGE";
  return "GREEN";
}
__name(inferSeverityFromText, "inferSeverityFromText");
function parseReliefwebFeed(xml) {
  const parser = new import_fast_xml_parser2.XMLParser({ ignoreAttributes: false, attributeNamePrefix: "" });
  const doc = parser.parse(xml);
  const items = doc?.rss?.channel?.item;
  if (!items) return [];
  const list = Array.isArray(items) ? items : [items];
  ...
  for (const it of list) {
    ...
    const severity = inferSeverityFromText(fullText, disaster_type);
    const affected_population = extractAffectedPopulation2(fullText);
    result.push({
      external_id: ext,
      disaster_type,
      severity,
      title,
      country: countryName && resolveCountryIso2(countryName) || void 0,
      event_timestamp: when.toISOString(),
      description,
      affected_population
    });
...
__name(parseReliefwebFeed, "parseReliefwebFeed");

// src/usgs.ts
var import_fast_xml_parser3 = __toESM(require_fxp(), 1);
function extractMagnitude(title) {
  const magnitudeMatch = title.match(/M\s*(\d+(?:\.\d+)?)/i);
  return magnitudeMatch ? parseFloat(magnitudeMatch[1]) : null;
}
__name(extractMagnitude, "extractMagnitude");
function extractPagerSeverity(summary) {
  if (/pager-red|PAGER.*?RED/i.test(summary)) return "RED";
  if (/pager-orange|PAGER.*?ORANGE/i.test(summary)) return "ORANGE";
  if (/pager-yellow|PAGER.*?YELLOW/i.test(summary)) return "ORANGE";
  return "GREEN";
}
__name(extractPagerSeverity, "extractPagerSeverity");
...
__name(extractLocationFromTitle, "extractLocationFromTitle");
function extractDepth(summary) {
  const depthMatch = summary.match(/<dt>Depth<\/dt><dd>(\d+(?:\.\d+)?)\s*km/i);
  return depthMatch ? parseFloat(depthMatch[1]) : null;
}
__name(extractDepth, "extractDepth");
...
function estimateAffectedPopulation(magnitude, depth, summary) {
  if (!magnitude) return void 0;
  ...
  if (isRed && magnitude >= 7) return 1e6;
  if (isRed && magnitude >= 6) return 5e5;
  if (isOrange && magnitude >= 6.5) return 1e5;
  if (isOrange && magnitude >= 5.5) return 5e4;
  if (isYellow && magnitude >= 6) return 25e3;
  if (magnitude >= 7) return 1e4;
  if (magnitude >= 6) return 5e3;
  return void 0;
}
__name(estimateAffectedPopulation, "estimateAffectedPopulation");
function parseUSGSFeed(xmlText) {
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        const affected_population = magnitude ? estimateAffectedPopulation(magnitude, depth, summary) : void 0;
        let country;
        ...
        results.push({
          external_id,
          disaster_type: "earthquake",
          // All USGS feeds are earthquakes
          severity,
          title,
          country,
          coordinates_lat,
...
__name(parseUSGSFeed, "parseUSGSFeed");

// src/noaa-cap.ts
var import_fast_xml_parser4 = __toESM(require_fxp(), 1);
function parseNOAACAPFeed(xmlContent) {
  try {
...
__name(parseNOAACAPFeed, "parseNOAACAPFeed");
function parseNOAACAPEntry(entry) {
  const id = entry.id || "";
  const title = entry.title || "";
  ...
  if (title.toLowerCase().includes("test message") || summary.toLowerCase().includes("monitoring message") || summary.toLowerCase().includes("please disregard")) {
    return null;
  }
  ...
  const disaster = {
    ...
    coordinates_lat: location.latitude || void 0,
    coordinates_lng: location.longitude || void 0,
    country: location.country,
    affected_population: estimateAffectedPopulation2(capSeverity, capUrgency, capAreaDesc)
  };
  return disaster;
}
__name(parseNOAACAPEntry, "parseNOAACAPEntry");
function mapCAPEventToDisasterType(capEvent, title) {
  ...
  if (eventText.includes("red flag") || eventText.includes("fire weather") || eventText.includes("extreme fire")) {
    return "wildfire";
  }
  if (eventText.includes("hurricane") || eventText.includes("tropical storm") || eventText.includes("typhoon") || eventText.includes("cyclone")) {
    return "cyclone";
  }
...
__name(mapCAPEventToDisasterType, "mapCAPEventToDisasterType");
function mapCAPSeverityToOurs(severity, urgency, title) {
...
__name(mapCAPSeverityToOurs, "mapCAPSeverityToOurs");
function extractLocationFromCAP(areaDesc, polygon) {
  let country = "US";
  let latitude = null;
  let longitude = null;
...
__name(extractLocationFromCAP, "extractLocationFromCAP");
function parsePolygonCoordinates(polygon) {
  const coords = [];
...
__name(parsePolygonCoordinates, "parsePolygonCoordinates");
function estimateLocationFromAreaDesc(areaDesc) {
  const areaLower = areaDesc.toLowerCase();
  const locationMap = {
...
__name(estimateLocationFromAreaDesc, "estimateLocationFromAreaDesc");
...
__name(extractAlertIdFromUrl, "extractAlertIdFromUrl");
function estimateAffectedPopulation2(severity, urgency, areaDesc) {
  let basePopulation = 1e4;
  ...
  if (areaDesc) {
    const areaLower = areaDesc.toLowerCase();
    if (areaLower.includes("los angeles") || areaLower.includes("new york") || areaLower.includes("chicago") || areaLower.includes("houston")) {
      basePopulation *= 10;
...
__name(estimateAffectedPopulation2, "estimateAffectedPopulation");

// src/nasa-firms.ts
function parseFIRMSResponse(csvContent) {
  try {
...
__name(parseFIRMSResponse, "parseFIRMSResponse");
function parseFIRMSRecord(record) {
  try {
    const latitude = parseFloat(record.latitude);
    const longitude = parseFloat(record.longitude);
    const confidence = parseInt(record.confidence || "0", 10);
    const brightness = parseFloat(record.brightness || "0");
    const frp = parseFloat(record.frp || "0");
    const acq_date = record.acq_date;
    const acq_time = record.acq_time;
...
__name(parseFIRMSRecord, "parseFIRMSRecord");
function determineCountryFromCoordinates(lat, lng) {
...
__name(determineCountryFromCoordinates, "determineCountryFromCoordinates");
async function fetchFIRMSGlobalData(mapKey, daysBack = 1) {
  try {
...
__name(fetchFIRMSGlobalData, "fetchFIRMSGlobalData");
...
async function processParsedEmail(parsed, env) {
  try {
    ...
    let useLegacy = false;
    try {
      existing = await env.DB.prepare("SELECT id, severity FROM disasters WHERE external_id = ?").bind(parsed.external_id).first();
    } catch (err) {
      if (String(err?.message || err).toLowerCase().includes("no such column: external_id")) {
        useLegacy = true;
        existing = await env.DB.prepare("SELECT id, severity FROM disasters WHERE id = ?").bind(parsed.external_id).first();
      } else {
        throw err;
      }
    }
    ...
    if (!existing) {
      if (useLegacy) {
        ...
        await env.DB.prepare(`INSERT INTO disasters (external_id, disaster_type, severity, title, country, coordinates_lat, coordinates_lng, event_timestamp, description, affected_population)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(parsed.external_id, normType, parsed.severity, parsed.title, parsed.country || null, parsed.coordinates_lat ?? null, parsed.coordinates_lng ?? null, parsed.event_timestamp, parsed.description || null, parsed.affected_population ?? null).run();
      }
      newDisasters = 1;
    } else {
      if (useLegacy) {
        ...
        await env.DB.prepare(`UPDATE disasters SET disaster_type = ?, severity = ?, title = ?, country = ?, coordinates_lat = ?, coordinates_lng = ?, event_timestamp = ?, description = ?, affected_population = ?, updated_at = CURRENT_TIMESTAMP WHERE external_id = ?`).bind(normType, parsed.severity, parsed.title, parsed.country || null, parsed.coordinates_lat ?? null, parsed.coordinates_lng ?? null, parsed.event_timestamp, parsed.description || null, parsed.affected_population ?? null, parsed.external_id).run();
      }
      if (existing.severity !== parsed.severity) {
        await env.DB.prepare(`INSERT INTO disaster_history (disaster_id, severity_old, severity_new, change_reason)
                              VALUES (?, ?, ?, ?)`).bind(existing.id, existing.severity, parsed.severity, "email_update").run();
      }
      updatedDisasters = 1;
    }
    if (env.CACHE) {
      const criticalKeys = [
        "disasters:summary",
        "disasters:current:all:all:all:none:50:0"
        // Most common query
      ];
...
__name(processParsedEmail, "processParsedEmail");
var index_default = {
  async fetch(req, env) {
    ...
    if (req.method === "POST" && pathname === "/ingest/gdacs") {
      try {
        ...
        const res = await fetch(rssUrl, { cf: { cacheTtl: 60, cacheEverything: true } });
        ...
        let updatedDisasters = 0;
        const errors = [];
        const processChunk = /* @__PURE__ */ __name(async (chunk) => {
    ...
    if (req.method === "POST" && pathname === "/ingest/reliefweb") {
      try {
        ...
        const res = await fetch(rssUrl, { cf: { cacheTtl: 120, cacheEverything: true } });
    ...
    if (req.method === "POST" && pathname === "/ingest/noaa-cap") {
      try {
        ...
        const xml = await res.text();
        const items = parseNOAACAPFeed(xml).slice(0, 20);
        let newDisasters = 0;
        let updatedDisasters = 0;
        const errors = [];
        const processChunk = /* @__PURE__ */ __name(async (chunk) => {
    ...
    if (req.method === "POST" && pathname === "/ingest/nasa-firms") {
      try {
        ...
        const items = await fetchFIRMSGlobalData(mapKey, 1);
        let newDisasters = 0;
        let updatedDisasters = 0;
        const errors = [];
        const processChunk = /* @__PURE__ */ __name(async (chunk) => {
    ...
    if (req.method === "POST" && pathname === "/ingest/email") {
      ...
      await env.DB.prepare(`INSERT INTO processing_logs (email_date, disasters_processed, new_disasters, updated_disasters, status, processing_time_ms, email_size_bytes)
                            VALUES (?, ?, ?, ?, 'OK', ?, ?)`).bind((/* @__PURE__ */ new Date()).toISOString().slice(0, 10), parsedMany.length, newDisasters, updatedDisasters, Date.now() - start, subject.length + body.length).run();
      return json({ success: true, data: { processed: parsedMany.length, newDisasters, updatedDisasters } });
    }
    if (req.method === "GET" && pathname === "/health") {
      return json({ success: true, data: { status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() } });
    }
  ...
  async email(message, env, ctx) {
    try {
      const allow = (env.ALLOW_FROM || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
      ...
      await env.DB.prepare(`INSERT INTO processing_logs (email_date, disasters_processed, new_disasters, updated_disasters, status, processing_time_ms, email_size_bytes)
                            VALUES (?, ?, ?, ?, 'OK', ?, ?)`).bind((/* @__PURE__ */ new Date()).toISOString().slice(0, 10), parsedMany.length, newDisasters, updatedDisasters, Date.now() - t0, rawBuf.byteLength).run();
    } catch (err) {
      try {
        await env.DB.prepare(`INSERT INTO processing_logs (email_date, disasters_processed, new_disasters, updated_disasters, status, processing_time_ms, email_size_bytes)
                              VALUES (?, 0, 0, 0, 'ERROR', 0, 0)`).bind((/* @__PURE__ */ new Date()).toISOString().slice(0, 10)).run();
      } catch {
      }
  ...
  async scheduled(event, env, ctx) {
    try {
      ...
      const [gdacsRes, reliefRes, usgsRes, noaaCapRes] = await Promise.all([
        fetch(gdacsUrl, { cf: { cacheTtl: 300, cacheEverything: true } }),
        fetch(reliefUrl, { cf: { cacheTtl: 300, cacheEverything: true } }),
        fetch(usgsUrl, { cf: { cacheTtl: 300, cacheEverything: true } }),
        fetch(noaaCapUrl, {
          headers: {
            "User-Agent": "Flare360-DisasterMonitor/1.0 (+https://flare360.samuel-1e5.workers.dev)",
            "Accept": "application/atom+xml, application/xml, text/xml, */*",
      ...
      if (gdacsRes.ok) {
        ...
        await Promise.all(items.map((p) => processParsedEmail(p, env)));
      }
      if (reliefRes.ok) {
      ...
      if (env.NASA_FIRMS_MAP_KEY) {
        try {
          ...
          for (let i = 0; i < firmsItems.length; i += 5) {
            ...
            await Promise.all(batch.map((p) => processParsedEmail(p, env)));
...
function stripHtmlToPlain(html) {
  ...
  const entities = noTags.replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">");
  return entities.replace(/[\t\x0B\f\r]+/g, " ").replace(/\n\s*\n\s*/g, "\n\n").trim();
}
__name(stripHtmlToPlain, "stripHtmlToPlain");
export {
