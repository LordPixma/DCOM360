...
// .wrangler/tmp/bundle-M8Hhu6/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-M8Hhu6/middleware-insertion-facade.js
init_modules_watch_stub();
...
var src_default = {
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      await Promise.all(items.map(async (p) => processParsedEmail(p, env)));
      if (env.CACHE) {
        const keys = ["disasters:summary", "disasters:current:all:all:all:50:0", "disasters:history:7", "countries:list"];
        await Promise.all(keys.map((k) => env.CACHE.delete(k).catch(() => {
        })));
