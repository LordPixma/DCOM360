...
// .wrangler/tmp/bundle-5amZjx/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-5amZjx/middleware-insertion-facade.js
init_modules_watch_stub();
...
// src/volcanodiscovery.ts
init_modules_watch_stub();
var import_fast_xml_parser2 = __toESM(require_fxp(), 1);
function inferTypeFromVD(title, description) {
...
__name(inferTypeFromVD, "inferTypeFromVD");
...
function parseVolcanoDiscoveryFeed(xml) {
  ...
  for (const it of list) {
    ...
    const title = String(it.title || "");
    const description = it.description ? String(it.description) : void 0;
    const pubDate = it.pubDate ? new Date(it.pubDate) : /* @__PURE__ */ new Date();
    const when = isNaN(pubDate.getTime()) ? /* @__PURE__ */ new Date() : pubDate;
    let countryIso;
...
__name(parseVolcanoDiscoveryFeed, "parseVolcanoDiscoveryFeed");
    ...
    if (req.method === "POST" && pathname === "/ingest/gdacs") {
      try {
        ...
        for (const p of items) {
          ...
          if (r.error) errors.push({ id: p.external_id, error: r.error });
        }
        if (env.CACHE) {
  ...
  async scheduled(event, env, ctx) {
    try {
      ...
      const [gdacsRes, vdRes] = await Promise.all([
        ...
        fetch(vdUrl, { cf: { cacheTtl: 300, cacheEverything: true } })
      ]);
      if (gdacsRes.ok) {
